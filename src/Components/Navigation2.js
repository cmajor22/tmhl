    ...
    return (
            <Drawer anchor="left" open={isOpen} onClose={() => dispatch(toggleMenu())} sx={classes.drawer}>
                <Box sx={classes.drawerContent}>
                    <Paper elevation={0} square sx={classes.menuSingleItem}>
                        <Grid container>
                            <Grid item xs={11}>
                    ...
                    {menuItems.map((item) => {
                        if(item.children.length === 0) {
                            return <Paper key={item.label} elevation={0} square sx={classes.menuSingleItem}>
                                <Typography onClick={() => {goPage(item.target)}}>{item.label}</Typography>
                            </Paper>
                        }else{
                            return <Accordion key={item.label} elevation={0} square sx={classes.menuMultiItem}
                            sx={{
                                '&:before': {
                                    display: 'none',
                                }
                    ...
                    <Paper elevation={0} square sx={classes.menuSingleItem}></Paper>
                </Box>
            </Drawer>
    )
}
