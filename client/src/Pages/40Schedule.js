...
function Schedule40(props) {
    ...
    let fixedWidthTime = isMobile ? 60 : 80;
    ...
    const gamesColumns = isMobile ? [
        ...
        { field: 'time', headerName: 'TIME', sortable: false, width: fixedWidthTime, headerAlign: 'center', align: 'center' },
    ...
    useEffect(() => {
        ...
        schedule.scheduleGames.forEach((game) => {
            sched.push({
                id: i,
                ...game,
                homeGoals: game.uploaded ? game.homeGoals : "-",
                awayGoals: game.uploaded ? game.awayGoals : "-",
            });
            i++;
        });
    ...
    return <Container sx={{paddingBottom: '15px'}}>
        {schedule.scheduleGamesLoading ?
            ...
            :
            [
                <Box key={0} sx={{backgroundColor: 'rgba(0, 0, 0, 0.7)', backdropFilter: 'blur(5px)', padding: '5px', marginBottom: '15px'}}>
                ...
                <Box key={1} sx={{backgroundColor: 'rgba(0, 0, 0, 0.7)', backdropFilter: 'blur(5px)', padding: '5px'}}>
                    <TmhlTable
                        rows={filteredGames}
